                                                ...
                                                )
    {
        ...
        $eventDispatcher->listen(
            ...
            function(GetPaymentMethodContent $event) use($basketRepository, $paymentHelper, $paymentService, $sessionStorage, $twig, $settingsService, $response, $paymentController) {
                ...
                if((empty($paymentRequestData['paymentRequestData']['customer']['first_name']) && empty($paymentRequestData['paymentRequestData']['customer']['last_name'])) || empty($paymentRequestData['paymentRequestData']['customer']['email'])) {
                   ...
                   if(empty($paymentRequestData['paymentRequestData']['customer']['email'])){
                    ...
                    || ($paymentKey == 'NOVALNET_GUARANTEED_INVOICE' && $showBirthday == false)) {
                        ...
                        $contentType = 'htmlContent';
                    } elseif ($sessionStorage->getPlugin()->getValue('expressWallet') == 'expressWallet') {
                        $content = '';
                        $contentType = 'continue';
                    }
                }
                ...
                $expressCheckout = 0;    
                if($sessionStorage->getPlugin()->getValue('expressWallet') == 'expressWallet') {
                    $expressCheckout = 1;
                    ...
                    $nnPaymentData['paymentRequestData']['transaction']['amount'] = $paymentHelper->convertAmountToSmallerUnit($basketRepository->load()->basketAmountNet);
