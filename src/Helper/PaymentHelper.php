...
class PaymentHelper
{
    ...
    {
		$this->getLogger(__METHOD__)->alert('Novalnet::paymentResponseData', $paymentResponseData);
        try {
            /** @var Payment $payment */
            ...
            $payment->amount          = ($paymentResponseData['result']['status'] == 'SUCCESS' && (in_array($this->eventData['transaction']['refund']['payment_type'], ['INSTALMENT_INVOICE_BOOKBACK', 'INSTALMENT_SEPA_BOOKBACK'])) && !empty($paymentResponseData['transaction']['refund']['amount'])) ? ($paymentResponseData['transaction']['refund']['amount'] / 100) : (($paymentResponseData['instalment']['cycle_amount']) ? (($paymentResponseData['instalment']['cycle_amount']) / 100) : ($paymentResponseData['transaction']['status'] == 'CONFIRMED' ? ($paymentResponseData['transaction']['amount'] / 100) : 0));
