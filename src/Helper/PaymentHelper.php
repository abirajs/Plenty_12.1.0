...
class PaymentHelper
{
    ...
    {
         $this->getLogger(__METHOD__)->alert('Novalnet::createPlentyPayment001', $paymentResponseData);
        try {
            /** @var Payment $payment */
            ...
            $payment->amount          = ($paymentResponseData['result']['status'] == 'SUCCESS' && empty($paymentResponseData['instalment']['cancel_type'])   && !empty($paymentResponseData['transaction']['refund']['amount'])) ? ($paymentResponseData['transaction']['refund']['amount'] / 100) : (($paymentResponseData['result']['status'] == 'SUCCESS' && $paymentResponseData['instalment']['cycle_amount']) ? (($paymentResponseData['instalment']['cycle_amount']) / 100) : ($paymentResponseData['transaction']['status'] == 'CONFIRMED' ? ($paymentResponseData['transaction']['amount'] / 100) : 0));
