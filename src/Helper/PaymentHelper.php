...
class PaymentHelper
{
	...
	$this->getLogger(__METHOD__)->alert('createPlenty::paymentResponseData', $paymentResponseData);
        try {
            ...
            $payment->amount          = ($paymentResponseData['result']['status'] == 'SUCCESS' && !empty($paymentResponseData['transaction']['refund']['amount'])) ? ($paymentResponseData['transaction']['refund']['amount'] / 100) : ($paymentResponseData['transaction']['status'] == 'CONFIRMED' ? ($paymentResponseData['transaction']['amount'] / 100) : 11.11);
