...
class PaymentHelper
{
    ...
    {
        try {
			$this->getLogger(__METHOD__)->error('createPlentyPayment paymentResponseData ' . $paymentResponseData['transaction']['order_no'], $paymentResponseData);
            /** @var Payment $payment */
            ...
            $bookingText    = isset($paymentResponseData['bookingText']) ? $paymentResponseData['bookingText'] : (!empty($paymentResponseData['transaction']['tid']) ? 'TID: ' . $paymentResponseData['transaction']['tid'] . "<br>" .  $paymentResponseData['transaction']['test_mode'] : $paymentResponseData['tid']);
