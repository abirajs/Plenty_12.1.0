...
class PaymentService
{
    ...
    public function getPaymentData(&$paymentRequestData, $paymentKey)
    {
        ...
        $this->getLogger(__METHOD__)->error('onhold_amountnn', $this->settingsService->getPaymentSettingsValue('onhold_amount', strtolower($paymentKey)));
        $this->getLogger(__METHOD__)->error('allow_B2B_customernn', $this->settingsService->getPaymentSettingsValue('payment_action', strtolower($paymentKey)));
        $this->getLogger(__METHOD__)->error('instament_cyclesnn', $this->settingsService->getPaymentSettingsValue('payment_action', strtolower($paymentKey)));
        ...
        if(in_array($paymentKey, ['NOVALNET_SEPA', 'NOVALNET_CC', 'NOVALNET_INVOICE', 'NOVALNET_APPLEPAY', 'NOVALNET_GUARANTEED_INVOICE', 'NOVALNET_GUARANTEED_SEPA', 'NOVALNET_INSTALMENT_SEPA', 'NOVALNET_INSTALMENT_INVOICE', 'NOVALNET_PAYPAL', 'NOVALNET_GOOGLEPAY']) && !empty($this->settingsService->getPaymentSettingsValue('payment_action', strtolower($paymentKey)))) {
            ...
            $this->getLogger(__METHOD__)->error('onholdurllliff', $paymentKey);
            ...
            if(empty($authorizeAmount) || (!empty($authorizeAmount) && is_numeric($authorizeAmount) && $paymentRequestData['transaction']['amount'] > $authorizeAmount)) {
               ...
               $this->getLogger(__METHOD__)->error('onholdurllliffinniff', $paymentUrl);
            }
        }
        ...
        if(in_array($paymentKey, ['NOVALNET_SEPA'])) {
            $dueDate = $this->settingsService->getPaymentSettingsValue('due_date', strtolower($paymentKey));
    ...
    */
    ...
    public function performServerCall()
    {
       ...
       if($this->settingsService->getPaymentSettingsValue('novalnet_order_creation') == true || !empty($nnOrderCreator) || ($nnReinitiatePayment == 1)) {
        ...
        if($this->isRedirectPayment($paymentKey) || !empty($nnDoRedirect) || !empty($nnMBwayDoRedirect) || (!empty($nnGooglePayDoRedirect) && (string) $nnGooglePayDoRedirect === 'true')) {
        ...
        $billingAddressId = !empty($basket->customerInvoiceAddressId) ? $basket->customerInvoiceAddressId : $this->sessionStorage->getPlugin()->getValue('nnBillingAddressId');
            ...
            if(!is_null($basket) && $basket instanceof Basket && !empty($billingAddressId)) {
                ...
                if($this->settingsService->getPaymentSettingsValue('payment_active', $paymentKey) == true) {
                    ...
                    && in_array($billingShippingDetails['billing']['country_code'], $this->getEuropeanRegionCountryCodes()) && $billingAddress->companyName	))
                    && (!empty($basket->currency) && $basket->currency == 'EUR')
                    && (!empty($minimumInstalmentAmount) &&  (int) $minimumInstalmentAmount <= (int) $basketAmount)) {
                        // If the guaranteed conditions are met, display the guaranteed payments
                        return 'instalment';
                    }
                    // If none matches, error message displayed
    ...
    */
    ...
    {
	   $this->getLogger(__METHOD__)->error('getInstalmentInformation', $transactionData);
       $InstalmentComments  = '';
