...
class PaymentService
{
    ...
    */
    ...
    public function performServerCall()
    {
        ...
        $this->getLogger(__METHOD__)->alert('paymentRequestData', $paymentRequestData['paymentRequestData']);
        ...
        $this->getLogger(__METHOD__)->alert('paymentResponseData', $paymentResponseData);
    ...
    */
	...
	$this->getLogger(__METHOD__)->error('Novalnet::getBankDetailsInformation', $transactionData);
	    ...
	    $cycleAmount = $this->paymentHelper->convertAmountToMinorUnit($transactionData['cycle_amount']);
            $invoiceComments = PHP_EOL . sprintf($this->paymentHelper->getTranslatedText('transfer_amount_duedate_text'), number_format((float)$transactionData['cycle_amount'] / 100 , 2), $transactionData['currency'], date('Y/m/d', (int)strtotime($transactionData['due_date'])));
            // If the transaction is in On-Hold not displaying the due date
            if($transactionData['tx_status'] == 'ON_HOLD') {
                $invoiceComments = PHP_EOL . PHP_EOL . sprintf($this->paymentHelper->getTranslatedText('transfer_amount_text'), number_format((float)$transactionData['cycle_amount'] / 100 , 2), $transactionData['currency']);
            }
        } else {
	    $amount = $this->paymentHelper->convertAmountToMinorUnit($transactionData['amount']);
            $invoiceComments = PHP_EOL . sprintf($this->paymentHelper->getTranslatedText('transfer_amount_duedate_text'), number_format((float)$transactionData['amount'], 2), $transactionData['currency'], date('Y/m/d', (int)strtotime($transactionData['due_date'])));
            // If the transaction is in On-Hold not displaying the due date
            if($transactionData['tx_status'] == 'ON_HOLD') {
                $invoiceComments = PHP_EOL . PHP_EOL . sprintf($this->paymentHelper->getTranslatedText('transfer_amount_text'), number_format((float)$transactionData['amount'], 2), $transactionData['currency']);
            }
        }
