...
class PaymentService
{
	...
	$this->getLogger(__METHOD__)->alert('enforce_3d', $this->settingsService->getPaymentSettingsValue('enforce', $paymentKey));	
		$this->getLogger(__METHOD__)->alert('enforce_3d1', $paymentKey);
		$this->getLogger(__METHOD__)->alert('enforce_3d2', strtolower((string) $paymentKey));
	}
	    
        if($paymentKey == 'NOVALNET_CC' && $this->settingsService->getPaymentSettingsValue('enforce', strtolower($paymentKey)) == true) {
    ...
    */
	...
	$this->getLogger(__METHOD__)->error('$transactionData', $transactionData);    
        ...
        if(!empty($transactionData['tid'])) {
            $transactionComments .= $this->paymentHelper->getTranslatedText('nn_tid') . $transactionData['ref_tid'] ?? $transactionData['tid'];
        }
	...
	$this->getLogger(__METHOD__)->error('Novalnet::getBankDetailsInformation', $transactionData);
        ...
        $invoiceComments .= PHP_EOL . $this->paymentHelper->getTranslatedText('payment_reference1'). 'TID '. $transactionData['ref_tid'] ?? $transactionData['tid'];
