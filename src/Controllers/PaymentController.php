...
{ 
    ...
    */
    ...
    public function paymentResponse()
    {
        ...
        $this->getLogger(__METHOD__)->error('Novalnet::$paymentResponseData', $paymentResponseData);
        ...
        if(!empty($paymentResponseData['tid'])) {
            ...
            $this->getLogger(__METHOD__)->error('Novalnet::$paymentResponseData2', $paymentResponseData);
    ...
    public function processPayment()
    {
        ...
        if(in_array($paymentRequestPostData['nn_payment_key'], ['NOVALNET_ACH'])) {
        ...
        if(in_array($paymentRequestPostData['nn_payment_key'], ['NOVALNET_MBWAY'])) {
        ...
        if(in_array($paymentRequestPostData['nn_payment_key'], ['NOVALNET_GUARANTEED_INVOICE', 'NOVALNET_GUARANTEED_SEPA','NOVALNET_INSTALMENT_SEPA','NOVALNET_INSTALMENT_INVOICE']) && !empty($paymentRequestPostData['nn_show_dob'])) {
        ...
        if(in_array($paymentRequestPostData['nn_payment_key'], ['NOVALNET_INSTALMENT_INVOICE', 'NOVALNET_INSTALMENT_SEPA']) && !empty($paymentRequestPostData['nn_show_dob'])) {
            $paymentRequestData['paymentRequestData']['instalment']['cycles'] = 2;
        }
        ...
        if(!empty($paymentRequestPostData['nn_reinitializePayment'])) {
            ...
            if($this->settingsService->getPaymentSettingsValue('novalnet_order_creation') != true) {
                ...
                if(!empty($paymentResponseData) && $paymentResponseData['result']['status'] != 'SUCCESS') {
                    ...
                    $this->sessionStorage->getPlugin()->setValue('nnMBwayDoRedirect', null);
    ...
    {   
        ...
        $this->sessionStorage->getPlugin()->setValue('nnDoRedirect', null);
        $this->sessionStorage->getPlugin()->setValue('nnGooglePayDoRedirect', null);
        if($this->paymentService->isRedirectPayment($paymentKey) || !empty($nnDoRedirect) || !empty($nnMBwayDoRedirect)  || (!empty($nnGooglePayDoRedirect) && (string) $nnGooglePayDoRedirect === 'true')) {
