        ...
        var requestData = {
            ...
            paymentIntent: {
                ...
                transaction: {
                    ...
                    amount: (String(jQuery('#nn_google_pay').attr('data-total-amount')) != '') ? String(jQuery('#nn_google_pay').attr('data-total-amount')) : (window.ceresStore.state.items[window.ceresStore.state.items.mainItemId].variation.documents[0].data.prices.default.price.value) * jQuery('.add-to-basket-container').find('input[type="text"], input[type="number"]').first().val() * 100,
